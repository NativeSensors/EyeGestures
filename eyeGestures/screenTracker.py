...
#     def process(self, point : (int,int), context_id : str):
...
class dataPoint():
    ...
    def getHeatMap(self):
        return Heatmap(
            self.getScreen().width,
            self.getScreen().height,
            self.buffor)
...
class EdgeDetector:
    ...
    def process(self, dataPoint, point_tracker, point_screen):
        (x_t,y_t) = point_tracker 
        (x_s,y_s) = point_screen

        new_w = dataPoint.getScreen().width
        # TODO: fix this to estimate w and h
        if(x_s <= 0 or x_s >= dataPoint.getDisplay().width):
            new_w = abs(dataPoint.getScreen().center().x - x_t) * 1 + 5
        ...
        new_h = dataPoint.getScreen().width
        if(y_s <= 0 or y_s >= self.height):
            new_h = abs(dataPoint.getScreen().center().y - y_t) * 1 + 5
...
class ScreenManager:
    ...
    def process(self, data):
        ...
        if cluster is not None:
            ...
            if data.calibration:
                self.screen_processor.update(data)
                self.backup_processor.update(data)

            p, percentage_main          = self.screen_processor.process(data)
            p_backup, percentage_backup = self.backup_processor.process(data)
...
class Screen:
    ...
    def scale(self, dataPoint, scale_w, scale_h, change = 0.5):
        
        dataPoint.getScreen().scale_h

        scale_diff_w = abs(dataPoint.getScreen().scale_w - scale_w)
        scale_diff_h = abs(dataPoint.getScreen().scale_h - scale_h)
        
        if scale_diff_w > change: 
            dataPoint.getScreen().scale_w = scale_w
            dataPoint.getScreen().width = dataPoint.getScreen().width/dataPoint.getScreen().scale_w * scale_w
        
        if scale_diff_h > change: 
            dataPoint.getScreen().scale_h = scale_h
            dataPoint.getScreen().heigth = dataPoint.getScreen().heigth/dataPoint.getScreen().scale_h * scale_h
        
    def scaleByStep(self,dataPoint,step_w=0.1,step_h=0.1):
        dataPoint.getScreen().scale_w = 1.0
        dataPoint.getScreen().scale_h = 1.0
        new_scale_w = float(dataPoint.getScreen().scale_w + step_w)
        new_scale_h = float(dataPoint.getScreen().scale_h + step_h)
        self.scale(dataPoint, new_scale_w, new_scale_h, 0.0)

    def convertToScreen(self, dataPoint):
        x,y = dataPoint.getLatest()[0],dataPoint.getLatest()[1]

        x = (x - dataPoint.getScreen().center().x) * ((x - dataPoint.getScreen().center().x) > 0)
        x = int((x/dataPoint.getScreen().width) * dataPoint.getDisplay().width)

        y = (y - dataPoint.getScreen().center().y) * ((y - dataPoint.getScreen().center().y) > 0)
        y = int((y/dataPoint.getScreen().height) * dataPoint.getDisplay().height)

        x = max(x,0)
        y = max(y,0)
        x = min(x,dataPoint.getDisplay().resolution_width)
        y = min(y,dataPoint.getDisplay().resolution_height)

        dataPoint.getScreen().buffor.add((x,y))
        return dataPoint.getScreen().buffor.getAvg()
    
    # Function to cut point to min and max of screen (not real screen resolution but virtaul one)
    def limitToScreen(self, dataPoint):
        margin = 10
        x = max(int(dataPoint.getScreen().center().x - margin),int(dataPoint.getLatestPoint()[0]))
        y = max(int(dataPoint.getScreen().center().y - margin),int(dataPoint.getLatestPoint()[1]))
        
        x = min(int(dataPoint.getScreen().center().x +
            dataPoint.getScreen().width  +
            margin),x)
        y = min(int(dataPoint.getScreen().center().y +
            dataPoint.getScreen().height +
            margin),y)
...
class ScreenProcessor:
    ...
    def __init__(self):
        ...
        # TODO: maybe this should be part of dataframe 
        self.scaling = True
    ...
    def process(self,dataPoints):
        ...
        if dataPoints.index > 20:
            new_screen_w,new_screen_h = self.edgeDetector.process(dataPoints,point,p)
            dataPoints.getScreen().width = new_screen_w
            dataPoints.getScreen().height = new_screen_h

        dataPoints.add(point)
        
        p += (dataPoints.getDisplay().offset_x, dataPoints.getDisplay().offset_y)

        # return how close that is hist region
        (_,_,roi_w,roi_h) = dataPoints.getHeatMap().getBoundaries()
        closeness_percentage = (roi_w * roi_h)/(dataPoints.getScreen().width * dataPoints.getScreen().height)
        return (p,closeness_percentage)

    # CALIBRATION FEATURE
    # This takes cluster center and uses it as screen center
    def update(self,dataPoint):

        heatmap = dataPoint.getHeatmap()
        (x,y) = heatmap.getCenter()
        # =====================================
        # ---------histogram obtained---------- 
        # =====================================

        dataPoint.getScreen().getCenter().x = x
        dataPoint.getScreen().getCenter().y = y
        # self.eyeScreen.setCenter(x,y) 
        self.__scale_up_to_cluster(
            dataPoint.getScreen().getCenter().width, 
            dataPoint.getScreen().getCenter().height, 
            dataPoint)

    def __scale_up_to_cluster(self, width, height, dataPoints):
        
        (_,_,cluster_w,cluster_h) = dataPoints.getCluster.getBoundaries()
